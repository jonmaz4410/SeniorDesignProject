...
def double_write(bar02, scd30, bmp, radiationWatch, date_var, time_var, ran_already, flag, ran_already_backup):
    ...
    with open(csv_path, 'a+', newline='') as SENSORS, open(backup_csv_path, 'a+', newline='') as backup_SENSORS:
        ...
        if not(ran_already_backup):
            ...
            try:
                backup_writer.writerow(['Date', 'Time', 'psi', 'temperature', 'altitude', 'CO2level', 'relativeHumidity', 'scdTemperature', 'bmp_temperature','bmp_pressure','bmp_altitude', 'duration', 'uSvh', 'uSvhError', 'cpm' ])
            
            except:
                print("Backup file error")
        ...
        if not(ran_already):
            ...
            try:
                transmit_writer.writerow(['Date', 'Time', 'psi', 'temperature', 'altitude', 'CO2level', 'relativeHumidity', 'scdTemperature', 'bmp_temperature','bmp_pressure','bmp_altitude','duration', 'uSvh', 'uSvhError', 'cpm'])
            
            except:
                print("Transmit file error")
        ...
        try:
            ...
            scdTemperature = scd30_data[1] #internal temperature
        ...
        try:
            bar_data = bar02.read()
            psi = bar_data[0]
            temperature = bar_data[1] #internal temperature
            altitude = bar_data[2] ##its inside, GET RID OF THIS
        except:
